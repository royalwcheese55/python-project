## concept question
1. What is Apache Spark, and how does it fit into the big data ecosystem (storage vscompute vs resource management)?
Apache Spark is a distributed compute engine that processes big data at scale; it does not store data (storage layer) and relies on systems like YARN or Kubernetes for resource management (resource layer).
...
3. What is the difference between transformations and actions? What happensinternally when an action is triggered?
...
An RDD (Resilient Distributed Dataset) is Spark’s core data structure — a fault-tolerant, distributed collection of records split into partitions that can be processed in parallel across a cluster. 
mmutability ensures safe parallel execution and lineage-based recovery, while partitioning enables scalable, parallel processing across the cluster.
...
9. Explain SCD Type 0, Type 1, and Type 2 in simple terms. When would you chooseeach one?
